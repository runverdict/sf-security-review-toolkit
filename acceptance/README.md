...
| `integration-pass-condition-<ver>.md` | The PRE-COMMITTED, write-before-run pass condition for a version's full-journey validation — authored before the run, graded **cold in a fresh restarted session** off disk. A clean pass gates that version's release **tag**. (`-0.6.0` [consented-scanner-install: gate → install → real evidence → asymmetric cleanup] and `-0.7.0` [throwaway-DAST: gate → stand-up → loopback-scoped active scan → asymmetric teardown + the credential contract] are the current open ones; `-0.5.1`/`-0.5.2` passed and their checkpoints are tagged through v0.5.5.) |
