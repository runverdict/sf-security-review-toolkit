...
| `solano-adjudication-key.md` | Solano's **sealed adjudication key** — per-issue intended call + why + the defensible-consistency bar (the calibration/precision grading key). Off-fixture (in the repo, never in `~/srt-solano`) so finders cannot read it. |
