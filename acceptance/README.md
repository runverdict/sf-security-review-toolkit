...
| `solano-adjudication-key.md` _(held off-repo during the Phase-2 cold-run window)_ | Solano's **sealed adjudication key** — per-issue intended call + why + the defensible-consistency bar (the calibration/precision grading key). **Temporarily relocated to `~/solano-adjudication-key.md` (off-repo)** for the cold run and removed from `main`, so the cold session — which works against a clone of `origin/main` — cannot read the answer key while it audits Solano; restored to this path after the cold-run window. (Stronger isolation than the Helios `expected-findings.md` precedent, on purpose, for the differentiating-value run.) |
