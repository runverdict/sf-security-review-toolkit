...
7b. **Agent-runtime trace probe (headless) — fill EXACTLY the gap the caveat above names.** The Apex smoke test proves the credential chain resolves; it does NOT exercise the Agentforce runtime (Apex egress ≠ Agentforce egress). With step 6's agent **activated** and its MCP tools registered (Manage Tools → **Save**), drive a scripted conversation against the agent and capture its execution trace — the "what can this agent actually DO / where does it egress" evidence:
   ...
   Requires step 6's **activated** agent + registered tools: an unactivated agent won't resolve `--api-name`, and if tools aren't registered the `actions` dimension is legitimately empty — the probe records that honestly as **"no observed actions"**, never a pass. **Same recorded `sf-deep-audit-ops` token — no new consent** (the engine's own `verifyConsent` fails closed before any `sf agent preview` spawn, because the sf-ops-gate-hook does not classify `agent preview` verbs — that guard is the only thing stopping an ungated conversation). It runs `agent preview start` → one `agent preview send` per validated utterance → `agent trace read --format detail --dimension actions|errors|routing` → `agent preview end`, redacts any secret-shaped value **before** write, and emits `.security-review/evidence/deployed-package/agent-trace-{actions,errors,routing}-<date>.json` (+ a names-only manifest). Run it with `cwd` inside the package DX project — `agent trace read` reads the local DX project's trace files. Coverage is only as good as the scripted utterance list (from `/sf-security-review-toolkit:prepare-test-environment`): the trace shows what the agent DID for those utterances, not the full reachable surface — dynamic evidence, still not Salesforce's live pen test. The live conversation against an activated agent is cold-run-validated (deferred to the midpoint cold run); the deterministic engine + its standing test are fully offline.
