...
│   ├── render-detected-elements.mjs # 0.8.25: VERBATIM detected-architecture-elements summary (WI-06/INV-15) — fixed {Element|Detected how (evidence)} table in CANONICAL_ELEMENT_ORDER (unknown types appended, never dropped) + listingType line over scope-manifest.json; missing → honest "scope not detected yet". 0.8.52: owns ELEMENT_TYPE_SYNONYMS + canonicalElementType — the SINGLE alias-map home (external-web-app/external-web/web-app/external-api/web-api → external-endpoint ONLY; unknown types returned unchanged); a synonym element sorts under its canonical slot, type rendered verbatim
...
│   ├── README.md
│   └── test-*.mjs                   # 61 dependency-free standing tests (827 checks) guarding the harness/ + hooks/ + CI hygiene
