...
- id: dast-scope-includes-identity-endpoints   # stable kebab id
  ...
  applies_to: [managed-package | external-endpoint | mcp-server | canvas | agentforce | all]
...
- **The ONE engine that downloads third-party tools onto the host is
  `install-scanners.mjs`, and it is split to keep the no-network rule intact
  (0.6.0).** Every other `harness/*.mjs` is pure, no-network, byte-identical —
  except the consent-gated, fail-closed live-op executors (the throwaway-DAST tier
  `standup-stack`/`run-dast`/`capture-openapi`/`teardown-stack`, the scratch-org
  tier `standup-org`/`teardown-org`/`capture-org-mcp`/`agent-trace-probe`/
  `normalize-agent-test`), each of which verifies its recorded consent in code,
  and the read-only `sf` query tier (`sf-autoresolve` — reaches Salesforce only
  through the already-authed CLI, gated by skill-level opt-in rather than a
  recorded token). `install-scanners.mjs` installs missing scan tools, so
...
- **Findings carry PROVENANCE; a deterministic engine's result is relayed, never
  ...
  (0.8.31) ships the first §10 per-scanner adapter — `checkov`** (every "§10" in this section means
  `docs/roadmap-deterministic-findings.md` §10, the adapter roadmap — not CONVENTIONS §10) (file-parser, `engine:'checkov'`,
  ...
  **Phase 2 (0.8.36) ships the sixth §10 adapter — `detect-secrets`** (file-parser,
  ...
  fixture) + `retire` + the tls/dast specials. **(0.8.118)** `pip-audit` joins as adapter #19
  (file-parser, `engine:'pip-audit'`, dep-CVE class-severity — the lockfile-less Python SCA leg);
  the `ADAPTERS` registry in `harness/ingest-scanner-findings.mjs` is authoritative for the
  current set (19, AD1-locked).
...
├── docs/                            # lifecycle-tracked docs — INDEX.md is the canonical index (§10)
├── baseline/
...
├── templates/                       # 17 reviewer-facing artifact templates + 2 schemas + operator/ render skeletons (0.8.23)
...
│   ├── api-endpoints-spec.md.tmpl   ├── exposed-tools-list.md.tmpl
│   ├── mcp-server-details.md.tmpl   # exposed-surface artifacts (generate-artifacts)
...
├── harness/                         # deterministic engines: no LLM, no deps, byte-identical, each test-backed (network/live exceptions are consent-gated — §7)
...
│   ├── capture-org-mcp.mjs          # 0.8.99 (S3): the THIRD, org-effective provenance lane for artifact-exposed-tools-list — mirrors standup-org (pure planMcpCapture + serverArgv(alias,id,verb) argv-builders + impure captureOrgMcp) reading the Agentforce API-Catalog via `sf agent mcp list/get/asset list` (no --status → DISCONNECTED enumerated too) to capture which registered MCP servers Salesforce ingested + their per-asset active/is-agent-action state (the unique activation evidence neither the code registry nor the raw tools/list reveals); NAMES/IDS-ONLY strict allowlist ({id,label,type,status,serverUrlHost} + assets{name,kind,active,isAgentAction} — serverUrl HOST-ONLY, token/authFields/session discarded); rides the recorded sf-deep-audit-ops consent (fails closed w/o it, NO new gate), opt-in --fetch live callout covered by the same token; org-effective counts CORROBORATE the code-registry N (never substitute it — reconciled in generate-artifacts step 4); empty catalog → honest no-mcp-servers; live leg cold-run-deferred
...
│   ├── dimension-registry.mjs       # 0.8.82: the ONE canonical source of valid audit-dimension keys (methodology/dimensions/*.md basenames — a new file IS its registration); consumers: build-audit-engine (hard scope-input gate) + render-target-map (display belt)
│   ├── sf-env.mjs                   # 0.8.95: shared library — makes every `sf` invocation JSON-safe (strips the CLI's stdout update banner); imported by the sf-facing engines, never driver-invoked
│   ├── normalize-agent-test.mjs     # 0.8.97: deterministic argv-builder + JSON→evidence normalizer for the headless `sf agent test` utterance-validation flow (prepare-test-environment step 9)
│   ├── agent-trace-probe.mjs        # 0.8.98: consented scripted conversation vs the installed+ACTIVATED Agentforce agent in the throwaway org — captures `sf agent trace read` (actions|errors|routing) as reviewer evidence
│   ├── sf-autoresolve.mjs           # 0.8.100: deterministic DevHub Tooling auto-resolve PRODUCER (render-sf-autoresolve.mjs is the frozen render half); read-only sf queries, writes only .security-review/sf-autoresolve.json
│   ├── rerender-runlog.mjs          # 0.8.104: re-derives the run-log's open-confirmed line AFTER dispositions — reproduces merge-ledger's severity print order byte-for-byte, never re-derives it elsewhere
│   ├── verify-report-headline.mjs   # 0.8.104: deterministic gate on the audit report's headline — the headline must count the ADJUDICATED band, not raw scanner noise
│   ├── seed-auto-dispositions.mjs   # 0.8.112: deterministic auto-disposition SEEDER — collapses known-safe scanner noise classes (the precision half of the deterministic-band honesty story)
│   ├── verify-dast-fired.mjs        # 0.8.115: deterministic DAST-fired tag gate — a real fire is a ZAP report on disk + provenance scanKind ≠ not-run (exit 0 fired · 2 degraded/absent)
│   ├── inject-report-headline.mjs   # 0.8.116: deterministic injection of the mandated cluster-headline block into the audit report — presence never depends on the synthesis LLM remembering to paste it
│   ├── assemble-submission-package.mjs # 0.8.118: deterministic submission-package assembler (compile-submission Step 10) — self-describing assembly: wizard-slot INDEX + PENDING-OWNER-RUN handoff + step-grouped artifacts
│   ├── detect-agentforce.mjs        # 0.8.123: deterministic Agentforce-element detector — journey preflight (Step 0.2) + scope-submission self-check source the `agentforce` signal from THIS one engine so the phases can never disagree
│   ├── emit-permission-set.mjs      # 0.8.122: self-skipping autorun permission-set gate — --check read-only (exit 0/2), --apply consent-gated fail-closed; curated REQUIRED_ALLOW (scoped prefixes, never Bash(*)); the live-op executors are EXCLUDED on purpose
...
│   ├── acceptance-report-2026-06-15.md   # dated cold-run acceptance report (HONEST-ARTIFACT)
│   ├── integration-pass-condition-*.md   # per-release cold-run pass conditions (0.5.1 · 0.5.2 · 0.6.0 · 0.7.0)
...
│   ├── README.md
│   └── test-*.mjs                   # 88 dependency-free standing tests (1377 checks) guarding the harness/ + hooks/ + CI + supply-chain hygiene
