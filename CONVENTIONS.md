...
- **Findings carry PROVENANCE; a deterministic engine's result is relayed, never
  ...
  **`tool→band`** adapter: unlike code-analyzer/checkov, Semgrep carries a real per-result severity
  ...
  inline `HIGH`/`LOW`/unknown synthetics). The remaining §10 adapters (build order 2a/2b — njsscan
  next, a DIFFERENT JSON shape but the same `tool→band` path) follow the same shape.
...
│   ├── README.md
│   └── test-*.mjs                   # 55 dependency-free standing tests (611 checks) guarding the harness/ + hooks/ + CI hygiene
