...
- **Findings carry PROVENANCE; a deterministic engine's result is relayed, never
  ...
  **`tool→band`** adapter: unlike code-analyzer/checkov, Semgrep carries a real per-result severity
  ...
  fixtures — a WARNING anchor + an ERROR anchor). The remaining §10 adapters (build order 2a/2b —
  bandit next, reusing this exact `tool→band` path) follow the same shape.
...
│   ├── README.md
│   └── test-*.mjs                   # 55 dependency-free standing tests (598 checks) guarding the harness/ + hooks/ + CI hygiene
