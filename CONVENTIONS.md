...
- **Findings carry PROVENANCE; a deterministic engine's result is relayed, never
  ...
  **Phase 2 (0.8.36) ships the sixth §10 adapter — `detect-secrets`** (file-parser,
  ...
  gate-label checks). The remaining §10 adapters (build order 2b) continue: **`trivy` next** — the heavier
  multi-mode one (container/IaC/SCA/secret), a mode-aware parse — then ext #3 (cross-engine dedup: OSV↔npm on
  the same CVE, now concrete with two dep-CVE engines) + `gosec` (needs a Go fixture) + the tls/dast specials.
