...
**The tag gate (DAST fires + `compile-submission` emits an SCI) is still never met — the DAST-fires
path is throwaway-MIRROR-only (rung 1 was RETIRED at 0.8.119, see above); the shipped path is the
prebuilt-image/build-from-source mirror ladder (0.8.109/0.8.114) gated by `verify-dast-fired.mjs`
(0.8.115).** The pre-cold-run items below have all since shipped:
...
~~B7 (gate-consolidation)~~ full-auto scope shipped 0.8.108 (prompt diet)**. One item at a time, each test-backed. Tag stays HELD until a clean cold run on
