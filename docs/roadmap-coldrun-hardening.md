...
- **`main` @ 0.8.101** — the quick-wins slices (0.8.81–0.8.94) + the cold-run bundle (consent/env/webport/devcve) + the S0 bootstrap agent-plugin pin (0.8.96) + the S1 headless agent-test evidence engine (0.8.97) + the S2 agent-trace-probe deployed-package runtime lens (0.8.98) + the S3 org-effective MCP tool-surface capture (0.8.99) + the R2 deterministic DevHub auto-resolve producer engine (0.8.100) + the R1 per-Bash-block `sf` update-banner guard (0.8.101), all merged; suite **76 files / 1159 checks** *(as of 0.8.101 — the current baseline is the **Current** pointer below, now 0.8.126)*, tag **HELD** (newest `v0.7.0`; since tagged `v0.9.0`, 2026-07-13 — see the tag-gate note below).
  ...
  **★ ALL SEQUENCED HARDENING SLICES ARE DONE. NEXT = the ★ MIDPOINT COLD RUN** *(since RAN — see the
  ★ POST-MIDPOINT-COLD-RUN PLAN section below)* — run the whole accumulated stack
...
**Current: `main` @ plugin `0.8.126`, suite 88 files / 1377 checks, untagged.** *(The section below
was written at 0.8.112; each item is now annotated with what shipped.)*

**The tag gate (DAST fires + `compile-submission` emits an SCI) has still never been met as a
cold-validated gate — the DAST-fires path is throwaway-MIRROR-only (rung 1 was RETIRED at 0.8.119,
see above); the shipped path is the prebuilt-image/build-from-source mirror ladder (0.8.109/0.8.114)
gated by `verify-dast-fired.mjs` (0.8.115). `v0.9.0` was tagged 2026-07-13 by owner election under
the ceiling test's §6 scoped claim WITHOUT this gate — the gate remains the bar for the
cold-validated claim, not for the scoped one (see `ceiling-test.md`'s addendum).** The pre-cold-run items below have all since shipped:
- ~~**DAST throwaway hardening (rung 2/3) — a root-caused CHAIN in `standup-stack.mjs` + `stack-detect.mjs`.**~~
  **DONE (0.8.114)** — G1 env-off-`recipe.file` + existsSync-gated `env_file:` + synthetic env +
  surfaced config stderr · G3 `--build` omitted on a genuinely-prebuilt recipe (+ `composeWebTierImage`
  fixed) · G4 `volumes: !reset []` per service (no host-credential mounts into the ZAP target) ·
  G2 documented as the honest external-managed-DB limitation. Test-backed (S12–S14 / U24–U25).
  *(Original chain, for the record: G1 fatal in-place `docker compose config` on a gitignored
  `env_file`; G3 forced `up -d --build`; G4 loopback override left `volumes:` mounted; G2 external
  `DATABASE_URL` can't stand up isolated.)*
- ~~**#3 headline-injection.**~~ **DONE (0.8.113 `merge-ledger` emits the deterministic
  `report-headline.md` sidecar + 0.8.116 `inject-report-headline.mjs` splices it; `verify-report-headline`
  stays the backstop)** — the report can no longer contradict the ledger's cluster headline.
- ~~**Two tag-gating acceptance tests:**~~ **DONE (0.8.115)** — `harness/verify-dast-fired.mjs` +
  `acceptance/test-verify-dast-fired.mjs` (the clean-checkout DAST-fires gate; the live fire itself is a
  live-docker runbook step, not hermetic) and `test-readiness-headline-integrity.mjs` (headline
  critical/high count matches the ADJUDICATED ledger, not the raw band).
...
8. ~~**★ MIDPOINT COLD RUN** — validate the ~15 accumulated test-backed-but-HERMETIC slices + real integration
   the hermetic tests can't reach; assess whether it justifies moving the held tag.~~ **RAN (2026-07-05, on
   cached 0.8.80 — see the ★ POST-MIDPOINT-COLD-RUN PLAN section).**
...
~~B7 (gate-consolidation)~~ full-auto scope shipped 0.8.108 (prompt diet)**. One item at a time, each test-backed. Tag stayed HELD until 2026-07-13, when `v0.9.0` was tagged by owner election under the ceiling
test's §6 scoped claim — the clean cold run remains the bar for the cold-validated claim (see
`ceiling-test.md`'s addendum).
...
### B7 — Structural gate-consolidation  *(consent-arch)* — **the full-auto scope SHIPPED via 0.8.108 (prompt diet: the launch re-ask is gone; Steps 2/3 fast-path key off the recorded run-mode + token — matching the INDEX row)**
