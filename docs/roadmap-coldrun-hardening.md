...
**Current: `main` @ plugin `0.8.109`, suite 80 files / 1205 checks, untagged.**

**The gate is unchanged and still never met.** Cold-run on `0.8.109` (bump + **push** first —
`claude plugin update` pulls `origin/main`, and a run against a stale cached plugin proves nothing).
The target's `.security-review/` and `docs/security-review/` **must be wiped**: the prior run's
`deterministic-dispositions.json` carries 18 scope-less entries that 0.8.103 now rejects by design,
and every finding in its ledger carries a `first_seen` the old assignment produced. If that run
comes back with **DAST fired** (evidence + the Slice-G provenance sidecar — note there is still no
zap/dast ingest adapter, so DAST findings do **not** enter the ledger band) **and**
`compile-submission` producing an SCI over an honest band, move `v0.7.0` → **`0.9.0`**. The tag
stays HELD until that clean run. Do NOT gate the tag on the deferred slices (sarif-band = own
slice; compile-digest = post-tag). The 0.8.109 run should be materially shorter (the
re-derivation-tax engines are fixed) and simpler (2 screens, not 13) than the 0.8.106 run.
