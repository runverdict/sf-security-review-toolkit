...
**Current: `main` @ plugin `0.8.105`, suite 76 files / 1159 checks, untagged.**

**The gate is unchanged.** Cold-run on `0.8.105` (bump + **push** first — `claude plugin update`
pulls `origin/main`, and a run against a stale cached plugin proves nothing). The target's
`.security-review/` and `docs/security-review/` **must be wiped**: the prior run's
...
comes back with **DAST fired** (evidence + the Slice-G provenance sidecar — note there is no
zap/dast ingest adapter yet, so DAST findings do **not** enter the ledger band) **and**
