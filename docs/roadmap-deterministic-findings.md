...
| `osv` ✅ / `npm-audit` ✅ / `trivy` / `retire` | file-parser | dep-CVE · container/IaC | scan-external-sca · scan-dependency-vulnerabilities | ✅ / ✅ / partial / ❌ | **CVSS→enum (fork)** — `osv` shipped 0.8.37 (**Extension A REALIZED**); `npm-audit` shipped 0.8.38 (the EASY Extension-A REUSE — a direct npm severity LABEL, no CVSS math); trivy next |
...
1. **Severity fork for dep-CVEs** *(✅ REALIZED 0.8.37, the `osv` adapter — Extension A).*
   ...
   `NPM_SEVERITY_TO_FINDING`, no CVSS math, gated by `scan-dependency-vulnerabilities`; `trivy`/`retire`
   reuse the same path with their own label maps.
...
- **2a (ingest-first, real fixtures):** checkov ✅ (shipped 0.8.31 — the FIRST 2a adapter:
  ...
  **Extension A: the CVSS→enum advisory-severity fork REALIZED**; a dep CVE carries a real CVSS while the
  ...
  per package, the band uses the PACKAGE severity not the first advisory's) → **trivy (next — the heavier
  multi-mode one: container/IaC/SCA/secret, needs its own mode-aware parse)** → then 2b (gosec [needs a Go
  fixture] / retire / cross-engine-dedup ext #3) + the tls/dast specials.
