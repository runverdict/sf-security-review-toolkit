...
- **Track 2 (the FINDINGS)** — written only by `merge-ledger.mjs` from the LLM
  finder→verifier `ledger_updates` (`workflow-template.mjs:566`). No scanner
  ingress; no `source`/`ruleId` field.

`compute-sci.mjs:235-239` ORs them: `blocked = openBlockerFindings (LLM) ||
...
existed anywhere. A hallucinated hand-off that dropped a real blocker. SFGE was
owner-gated when this campaign ran (`tool-detect.mjs` → `install:'owner'`/PENDING),
so it did not auto-run cold and the LLM dimension filled the vacuum
non-deterministically. **(Flipped 0.8.41 — see §5: the Code Analyzer stack is now a
consented tmp-install, so SFGE/PMD CRUD/FLS run cold and the band is deterministic by
default.)**
...
## 5. The SFGE owner-gating reality — RESOLVED 0.8.41 (cold-install milestone)

**Milestone (0.8.41):** Code Analyzer's CRUD/FLS flips from **owner-gated** to
**consented-deterministic-by-default**. `tool-detect.mjs` no longer marks the
`code-analyzer` family `install:'owner'`; it is a new `code-analyzer-stack` install
method, so `install-scanners.mjs` provisions the whole stack — the pinned
`@salesforce/cli` + the `code-analyzer` plugin (npm) + a JDK 11+ (a present `java`≥11
reused, else the pinned Temurin sha256-verified + extracted) — into the tmp root on the
SAME preflight consent as the OSS scanners. A spike validated the recipe live and proved
it hermetic: with the full contained env (`HOME`/`SF_*`/`TMPDIR`/`npm_config_cache`/
`JAVA_HOME` all under the tmp root) set BEFORE the npm install, the teardown's
`rm -rf <tmpRoot>` leaves zero residue, and the analyzer pins make the ledger band
byte-identical run-to-run. Footprint ~1 GB (+~320 MB if Java must be provisioned). The
ingest adapter needed NO change — it already extracts only stable fields.

**The honest fallback (unchanged posture).** Determinism is still **conditional on the
engine being runnable**: if the operator DECLINES the install, or the platform has no
pinned JDK and no present `java`≥11, SFGE's classes stay **PENDING-OWNER-RUN** (with the
clear "install `sf` + Code Analyzer to make these deterministic" prompt), NOT LLM-filled.
The flip changes the DEFAULT (cold → deterministic on consent); it does not remove the
fail-closed floor.

**Close-out (0.8.42).** The headline security guard — the JDK
sha256-verify-BEFORE-extract fail-closed — now has standing EXECUTOR coverage
(`test-install-scanners` `CA7`: a `file://` checksum mismatch → `failed` before
extract, the JDK never unpacked, npm never run, no PATH entry; mutation-proven
non-vacuous), and the `code-analyzer-stack` install branch gained the same
name-membership guard the other methods carry (`CA8`). The run-scans Family 1
prose was clarified so the engine-explicit `-r sfge` workspace form is the
PRIMARY deterministic-band command for BOTH a present `sf` and the cold stack —
the byte-verified HTML form stays the required submission artifact, but HTML
alone yields neither the FLS band nor the ingestable JSON.
...
(`docs/deterministic-findings-acceptance.md`) (Slice 3, 0.8.30). The Phase-1 residual was
the owner-gated act of actually running SFGE/Code Analyzer in the cold journey (§5) —
**RESOLVED 0.8.41**: the Code Analyzer stack is now a consented tmp-install, so the engines
run cold and the band is deterministic by default (PENDING-OWNER-RUN remains the fail-closed
fallback when the install is declined / no JDK is pinnable).
...
| `osv` ✅ / `npm-audit` ✅ / `pip-audit` ✅ (0.8.118) / `trivy` ✅ (config) / `retire` | file-parser | dep-CVE · IaC-misconfig | scan-external-sca · scan-dependency-vulnerabilities · scan-iac-misconfig | ✅ / ✅ / ✅ config / ❌ | **CVSS→enum (fork)** for dep-CVE · **class** for trivy config — `osv` shipped 0.8.37 (**Extension A REALIZED**); `npm-audit` shipped 0.8.38 (the EASY Extension-A REUSE — a direct npm severity LABEL, no CVSS math); `trivy` shipped 0.8.39 (**CONFIG / IaC-misconfig mode ONLY** — REUSES checkov's `iac-misconfig` class at class-severity, class-dispatch parse; **trivy SCA/secret modes still 2b**) |
