...
| A `global`/`@AuraEnabled` method with correct CRUD/FLS, sharing, and per-record checks that is simply broader than strictly needed | Least-privilege hardening (`low`/`medium`), not a `high` exploit — the access controls hold. Over-exposure is a finding when the breadth enables an *unguarded* path, not when the method is merely reachable. |
