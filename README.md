...
- **Pinned, digest-verified scanner installs.** The one engine that touches the network (`harness/install-scanners.mjs` — the documented exception, [`CONVENTIONS.md`](CONVENTIONS.md) §7) fails closed: raw binary downloads are version-pinned and sha256-verified against author-pinned checksums before any file is made executable or extracted; a digest mismatch aborts that tool rather than execute an unverified binary, and a tool/platform with no pin is skipped, never installed unverified. Package-manager installs (pip/npm/git) carry no per-file pin and rely on the manager's own integrity layer (PyPI / npm / Git-over-TLS).

Both properties are locked by standing checks (the `SC-*` checks in [`acceptance/test-ci-hygiene.mjs`](acceptance/test-ci-hygiene.mjs)): a tracked `package.json` or a third-party import in `harness/`/`hooks/` fails the build, and this section's claim is itself asserted present so the doc and the guard cannot drift apart. A machine-readable self-SBOM is a candidate future addition; it is not shipped today.
